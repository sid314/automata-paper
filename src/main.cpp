...
int makeNBD(int in, int jn)
{
    ...
    for (int i = in - 1; i < in + 2; i++)
    {
        ...
        {
            if (value[i * division + j] == 1 && i != 0 && j != 0 && i!= division && j!= division)
            {
                if (!(i == in && j == jn))
                {
...
void rule2()
{
    ...
    for (int i = 1; i < division-1; i++)
    {
        for (int j = 1; j < division-1 ; j++)
        {
            ...
            if (value[i * division + j] == 0 && makeNBD(i, j) == 3 && i != 0 && j != 0)
            {
                for (int k = 0; k < changers.size(); k++)
                {
                    //if (i != changers[k][0] || j != changers[k][1]);
                ...
                //std::cout << i << " , " << j << " # " << makeNBD(i, j) << std::endl;
                //sf::sleep(sf::milliseconds(80));
                //std::cout << firstNeightbours[0][0];
            

            //std::cout << makeNBD(i, j);
