...
void pattern(std::string filename, int rows = 4)
{
    ...
    for (int i = 0; i < seed.size(); i++)
    {
        ...
        if (seed[i] == "0")
        {
            kill(10 + succeser - ((seed.size() / rows) / 2), 10 + y - rows / 2);
        }
        else if (seed[i] == "1")
        {
            fill(10 + succeser - ((seed.size() / rows) / 2), 10 + y - rows / 2);
        }
        std::cout << seed[i];
    }
}
...
int main()
{
    ...
    pattern("C:\\Users\\HP\\Desktop\\New folder (2)\\automata-paper\\src\\try.txt", 16);
